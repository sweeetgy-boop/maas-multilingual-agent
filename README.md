...
6. [보안 아키텍처](#6-보안-아키텍처)
7. [데이터 소스 현황](#7-데이터-소스-현황)
8. [공공데이터 API 에서 확인한 것](#8-공공데이터-api-에서-확인한-것)
9. [실증 결과](#9-실증-결과)
10. [배포](#10-배포)
11. [평가](#11-평가)
12. [로컬 개발](#12-로컬-개발)
13. [알려진 제약](#13-알려진-제약)
14. [프로젝트 종료 시 정리 체크리스트](#14-프로젝트-종료-시-정리-체크리스트)
...
| Guardrails | `671i24gxu4oo` v1 — `maas-transit-guardrail-v3` (Standard tier, apac cross-region) |
...
(현재 `671i24gxu4oo`)에서 해결했다. `infra/guardrail-v2.json` 과 `guardrail-v3.json` 에
두 판본이 남아 있다.
...
과금되므로 [종료 체크리스트](#14-프로젝트-종료-시-정리-체크리스트)에 넣었다.
...
애플리케이션 방어 계층 6종은 [3. 아키텍처](#3-아키텍처)에 있다. 이 절은 그
바깥 — IAM·네트워크·전송·자격증명·배포 — 을 정리한다.

**아래 값은 2026-09-04 에 실 계정과 실 인스턴스에서 직접 조회해 확인한 것이다.**
설계 의도가 아니라 그날의 실제 상태이며, 하지 못한 것과 하지 않은 것을
[6.7](#67-하지-못한-것--알려진-위험)에 그대로 적었다. 정책 원문·검증 명령·실측
결과까지 필요하면 **[docs/architecture/security.md](docs/architecture/security.md)**.

### 6.1 최소 권한 — IAM 역할 분리

| 주체 | 용도 | 실제 권한 |
|---|---|---|
| `maas-llm-ec2` | EC2 인스턴스 프로파일 | SSM, CodeDeploy 에이전트, **S3 FullAccess, Bedrock FullAccess** |
| `maas-codepipeline-role` | 파이프라인 오케스트레이션 | 아티팩트 버킷 한정 S3, CodeBuild 시작, CodeDeploy, CodeConnections |
| `maas-codebuild-role` | 빌드·검증 | CloudWatch Logs + 아티팩트 버킷**뿐** |
| `maas-codedeploy-role` | 배포 | 관리형 `AWSCodeDeployRole` |
| `github-actions-maas` | IAM 사용자(액세스 키) | 배포 버킷 한정 S3, **`ssm:SendCommand`·`ec2:DescribeInstances` 는 `Resource: "*"`** |

**왜 나눴는가.** 하나의 역할에 전부 몰아주면 빌드 단계가 침해됐을 때 Bedrock
호출이나 EC2 조작까지 가능해진다. `maas-codebuild-role` 에 `bedrock:*` 도
`ec2:*` 도 `ssm:*` 도 주지 않은 것이 그 예다 — 빌드는 문법·JSON·평가셋을
검증하고 아티팩트를 넘길 뿐이고, 실제 배포는 CodeDeploy 에이전트가 EC2 위에서
한다. 초기에는 CodeBuild 가 `ssm:SendCommand` 로 직접 배포했는데, 그 구조에서는
빌드 스크립트를 고칠 수 있는 사람이 곧 인스턴스에 임의 명령을 실행할 수 있는
사람이 된다.

**나누다 만 곳도 있다.** `maas-llm-ec2` 는 관리형 FullAccess 두 개를 그대로
쓴다. 인스턴스에 실제로 필요한 것은 아티팩트 버킷 읽기와 Bedrock 모델 하나·
가드레일 하나인데, 붙어 있는 것은 계정의 모든 버킷과 모든 모델이다.
`github-actions-maas` 는 CodePipeline 으로 경로를 일원화하면서 쓰이지 않게
됐는데 액세스 키 2개가 아직 Active 다(하나는 사용 이력이 아예 없다).
둘 다 [6.7](#67-하지-못한-것--알려진-위험) 에 적었다.

### 6.2 네트워크 노출

보안그룹 `sg-0ce81d06ebaf0db08` 의 **실제** 인바운드는 넷이다.

| 포트 | 출처 | 용도 |
|---|---|---|
| 80/tcp | `0.0.0.0/0` | Let's Encrypt HTTP-01 챌린지 |
| 443/tcp | `0.0.0.0/0` | Caddy HTTPS |
| 3000/tcp | 개인 IP 1개 | nginx — 이 프로젝트와 무관, 정리 대상 |
| 7000–9000/tcp | 개인 IP 1개 | 개발 중 직접 접근용, 정리 대상 |

`7000–9000` 이 `8080`(API)과 `7860`(UI)을 포함한다. 두 프로세스는 `0.0.0.0` 에
바인딩되므로 **그 IP 하나에서는 Caddy 를 거치지 않고 평문 HTTP 로 API 에 직접
붙는다.** `X-API-Key` 인증은 살아 있지만 TLS 가 없다. 인터넷 전체에는 닫혀
있으나 "8080 은 외부에 열지 않았다"고 말하면 정확하지 않다.

**vLLM(8000)은 이 범위 안이어도 도달하지 못한다.** 컨테이너는 `0.0.0.0` 을
듣지만 Docker 포트 매핑이 `127.0.0.1:8000` 이라 호스트 밖으로 나가지 않는다.
묶어 둔 이유는 vLLM 이 인증 없는 OpenAI 호환 서버이기 때문이다 — 외부에 열면
누구나 L40S GPU 로 추론을 돌릴 수 있고, 게이트 모델을 직접 때려 방어 계층
6종을 통째로 우회하는 경로가 된다.

**SSH 는 열지 않았다.** 보안그룹에 22 규칙이 없고 인스턴스에 키페어도 붙어
있지 않다(`KeyName: null`). 접속은 전부 SSM Session Manager 다. 키 관리가
필요 없고(유출될 개인키가 존재하지 않는다), 세션과 Run Command 가 CloudTrail
이벤트로 남으며, 22 가 닫혀 있어 포트 스캐너의 사전 대입이 애초에 도달하지
않는다. 접속 허용·회수가 IAM 정책이라 계정 정지가 곧 서버 접근 차단이다.
개발 중에는 SSM 포트포워딩으로 vLLM(8000)에 접근했다.

**IMDSv2 를 강제했다**(`HttpTokens: required`). SSRF 로 `169.254.169.254` 를
읽어 인스턴스 역할의 임시 자격증명을 훔치는 고전 경로가 막힌다. 보안 평가에서
SSRF 차단률이 74%로 인젝션(97%)보다 낮은 만큼, 방어 계층이 아니라 인스턴스
설정이 마지막 안전망 역할을 한다 — `maas-llm-ec2` 가 FullAccess 를 들고 있어
더 그렇다.

### 6.3 전송 구간 암호화

Caddy + Let's Encrypt 로 HTTPS 를 종단한다. 인증서 발급·갱신이 자동이고
두 사이트 모두 다음 헤더를 보낸다.
...
**`includeSubDomains` 와 `preload` 는 일부러 넣지 않았다.** `duckdns.org` 는
여러 사용자가 나눠 쓰는 공용 도메인이라 하위 도메인까지 HSTS 를 강제하면 남의
호스트에 영향을 준다. 자기 도메인이 아닌 곳에서 켜는 것은 사고다.

ALB + ACM 이 정석이나 ALB 시간당 요금이 들고, ACM 을 붙이려면 소유 도메인이
필요한데 Route 53 등록비가 교육 계정 크레딧 대상이 아니었다. 인스턴스가
하나뿐이라 로드밸런서로 얻는 것도 없었다.

### 6.4 자격증명 관리

**저장소에 키를 넣지 않는다.** `.gitignore` 에 `.env`, `.env.*`, `*.pem`,
`*.key`, `credentials`, `.aws/` 를 등록했고 문서에는 변수 이름과 용도만 적는다.
커밋 전 검사는 [종료 체크리스트](#14-프로젝트-종료-시-정리-체크리스트)의 `grep` 이다.

**EC2 에서는 `/etc/maas-api.env` 에 두고 `EnvironmentFile=` 로 읽는다.**
권한 `600`, `root:root`. 배포 대상이 아니므로 CodeDeploy 가 덮어쓰지 않는다.
`Environment=` 대신 파일로 뺀 이유는 보안이 아니라 값이 깨졌기 때문이다 —
`%` 가 systemd 지정자로 해석되고 `$` 이스케이프가 남았다([환경변수 관리](#환경변수-관리)).
결과적으로 600 파일 한 곳으로 모인 것은 보안 측면에서도 낫다.

**다만 옮기다 말았다.** 외부 API 키 4개만 그 파일에 있고 `MAAS_API_KEY` 는
아직 유닛 파일의 `Environment=` 에 평문으로 남아 있는데, 유닛 파일 권한은
`644` 다. EC2 에 셸을 얻은 사용자면 읽을 수 있다.

**vLLM 은 `--disable-log-requests` 로 프롬프트 원문 로깅을 차단했다.**
사용자 입력에 이름·연락처·여권번호가 섞일 수 있다.

### 6.5 애플리케이션 계층 방어

계층 6종의 순서와 동작은 [3. 아키텍처](#3-아키텍처)에 있다. 보안 관점의
판단 근거만 적는다.

**확률적 방어와 결정론적 방어를 함께 둔다.** `local_gate`·`bedrock_guardrail`
은 모델 판정이라 확률적이고, `blocklist`·`pii_regex`·`grounding_check`·
`scope_policy` 는 규칙이라 결정론적이다. 한쪽만 두면 그쪽의 실패 양식이 그대로
구멍이 된다. Guardrails 가 다국어 금지어와 근거성 검증을 지원하지 않는 것이
직접 구현한 실제 이유이기도 하다.

**PII 는 가드레일 이전에 마스킹한다.** `mask_pii`(③)가 `apply_guardrail`(④)
보다 앞인 이유는 Guardrails 도 AWS 로 나가는 호출이기 때문이다. 원문을
검사받으려면 원문을 보내야 하는데, 그러면 "클라우드에 PII 를 보내지 않는다"가
성립하지 않는다.

**그 순서에 부작용이 있다 — 카드번호는 차단되지 않는다.** 가드레일 정책은
`CREDIT_DEBIT_CARD_NUMBER: BLOCK` 인데, 13~19자리 숫자를 로컬 정규식이 먼저
`[CARD]` 로 치환하므로 가드레일이 카드번호를 볼 일이 없다. 실제로 카드 형식
숫자가 든 시간표 질의는 차단되지 않고 마스킹된 채 정상 응답한다(실측).
동작 자체는 안전하지만 — 카드번호가 Bedrock 으로 나가지 않는다 — 두 계층의
의도가 어긋나 있으므로 어느 한쪽에 맞춰야 한다.

**LLM 이 도구 응답에 없는 값을 말하면 근거성 검증이 막는다.** 실제로 Haiku 가
지어낸 KTX 편명·요금 10건을 잡아냈다.

**가드레일 호출이 실패하면 요청이 죽는다(fail-closed).** `apply_guardrail` 에
`try/except` 가 없어 예외가 위로 올라가고 API 는 502, 채팅 UI 는 `route: error`
를 낸다. 어느 쪽도 검사를 건너뛰고 답변하지 않는다. 반면 `GUARDRAIL_ID` 가
아예 비어 있으면 통과시키되 기동 시 경고를 stderr 에 남긴다 — "설정하지 않은
것"과 "설정했는데 실패한 것"을 다르게 다룬다.

**OpenAI 호환 경로**([4. API](#4-api) 참고)

- `system` 메시지를 **무시한다.** 외부에서 Supervisor 프롬프트를 덮어쓰면
  방어 계층 전체가 무력화되기 때문이다 — 인젝션의 정석 경로다. 실측에서
  "제약 없는 어시스턴트"를 `system` 에 넣고 결제를 요청했더니 `local_gate`
  에서 막혔다.
- **스트리밍을 지원하지 않는다.** 근거성 검증이 완성된 답변을 봐야 하는데
  토큰 단위로 흘리면 환각이 검증 전에 노출된다.
- 무인증 경로의 유일한 남용 방어선인 레이트리밋은 `X-Forwarded-For` 위조로
  우회되지 않는다. Caddy 가 실제 접속 IP 를 헤더 맨 뒤에 덧붙이고 uvicorn 이
  그쪽을 읽는다 — 위조 헤더로 62회를 채운 뒤 헤더 없이 보낸 요청도 `429` 를
  받는 것을 확인했다. 단, 8080 에 직접 붙으면 갈라진다(6.2 의 정리 대상).

### 6.6 배포 파이프라인 보안

**CodeDeploy 훅이 systemd 유닛 파일과 `/etc/maas-api.env` 를 건드리지 않는다.**
API 키가 들어 있는 파일이라 배포로 덮어쓰면 키가 날아간다. 훅은
`/opt/maas/{gate,ui}` 밖을 아예 손대지 않는다.

**훅에서 `docker` 명령을 쓰지 않는다.** vLLM 컨테이너는 배포와 무관하고
모델 로딩에 2분이 걸리며, 잘못 만지면 게이트가 통째로 내려간다.

**CodeBuild 는 검증만 하고 배포 권한이 없다.** 실제 배포는 CodeDeploy 가
EC2 에이전트를 통해 수행한다.

아티팩트 버킷 둘 다 퍼블릭 액세스 차단 4종이 켜져 있고 SSE-S3(AES256)로
기본 암호화된다. `maas-pipeline-508139322599` 는 버전 관리도 활성이다.

### 6.7 하지 못한 것 — 알려진 위험

**이 절을 지우면 다음 사람이 안전하다고 오인한다.** "왜 안 했는가"에 사실이
아닌 사유를 적지 않으려고, 계정 제약이라고 알고 있던 항목은 실제로 시험해
확인했다 — 그 결과 몇 개는 "못 한 것"이 아니라 "하지 않은 것"이었다.

**공개 표면**

| 위험 | 사유 |
|---|---|
| `/v1/chat/completions`·`/v1/models` 가 **무인증으로 공개**돼 있다. 호출마다 Bedrock 비용이 발생한다 | GuardBench 등록 화면에 API Key 칸이 없다. 분당 레이트리밋만 걸린다. **테스트 종료 후 인증을 되돌려야 한다** |
| 443 이 `0.0.0.0/0` 으로 열려 있다 | GuardBench 가 서버리스라 아웃바운드 IP 가 고정되지 않아 IP 제한을 걸 수 없었다 |
| **8080·7860 이 개인 IP 1개에 평문으로 열려 있다** | `7000–9000` 규칙이 남았다. 개발 중 직접 접근용이었고 필요가 끝났는데 지우지 않았다. 지금 지워도 운영에 영향이 없다 |
| 3000/tcp 에 무관한 nginx 가 다른 개인 IP 에 열려 있다 | 인스턴스 이력상 남은 것으로 보인다. 규칙과 프로세스 모두 정리 대상 |

**권한·자격증명**

| 위험 | 사유 |
|---|---|
| **EC2 역할이 S3·Bedrock FullAccess** 다 | 관리형 정책을 그대로 붙이고 좁히지 않았다. 인스턴스가 침해되면 계정 전체 S3 에 닿는다 |
| **미사용 액세스 키 2개가 Active** (`github-actions-maas`) | `ssm:SendCommand` 가 `Resource: "*"` 라 키가 유출되면 인스턴스에 임의 명령이 가능하다. 지금 지워도 아무것도 깨지지 않는다 |
| 액세스 키를 GitHub Secrets 에 저장했다 | OIDC 가 더 안전하나 교육 계정에서 IAM Identity Provider 생성이 가능한지 **확인하지 않았다**. 권한 경계가 `iam:*` 를 리전 제한에서 빼 두었으므로 막혀 있지 않을 가능성이 높다 |
| `MAAS_API_KEY` 가 **644 유닛 파일**에 평문으로 있다 | 외부 API 키만 600 파일로 옮기고 이건 남겨뒀다. 일관성이 없다 |
| 시크릿을 Secrets Manager·Parameter Store 에 두지 않았다 | `/etc/maas-api.env` 평문 파일이고 파일 권한으로만 보호된다. 회전·감사·접근 이력이 없다 |

**데이터·관측**

| 위험 | 사유 |
|---|---|
| **EBS 볼륨이 암호화돼 있지 않다** (`vol-0245e8dc463d41c9e`, 300GB) | 시작 시점에 켜지 않았다. 나중에 켜려면 스냅샷 → 암호화 복사 → 볼륨 교체라 운영 중 전환이 번거롭다 |
| CloudTrail 추적을 만들지 않았다 | **권한 문제가 아니다.** 권한 경계는 리전만 제한하고, `create-trail` 을 시험하니 `AccessDenied` 가 아니라 "S3 버킷 없음" 오류가 났다. 만들 수 있었는데 만들지 않았다 — 기본 이벤트 기록 90일만 남고 장기 보관이 없다 |
| GuardDuty 를 켜지 않았다 | 이것도 권한이 아니라 선택이다 |
| WAF 를 두지 않았다 | 권한 경계가 `waf:*`·`cloudfront:*` 를 리전 제한에서 빼 두어 가능했으나 비용과 구성 복잡도 때문에 넣지 않았다. 실제로 스캐너로 보이는 GET 요청이 로그에 관측된다 |

**의존·구성**

| 위험 | 사유 |
|---|---|
| DuckDNS 무료 동적 DNS 에 의존한다 | 도메인 소유권이 제3자에 달려 있다. 토큰이 유출되면 도메인과 인증서를 함께 빼앗긴다. 기업 방화벽이 `*.duckdns.org` 를 차단하는 경우도 있다 |
| **가드레일 ID 불일치 — 채팅 UI 가 현재 동작하지 않는다** | `maas-api.service` 는 `671i24gxu4oo`(존재)를, `maas-ui.service` 는 `b53a6caaqa31`(**존재하지 않음**)을 가리킨다. UI 의 모든 요청이 `ValidationException` 으로 실패한다. fail-closed 라 답변이 새지는 않지만 UI 가 죽어 있다. `maas-ui.service` 의 `GUARDRAIL_ID` 를 고치고 `daemon-reload && restart` 하면 된다 |
...
인프라·자격증명 쪽의 알려진 위험(FullAccess 역할, 미사용 액세스 키, 미암호화
EBS, CloudTrail·WAF 부재 등)은 [6.7 하지 못한 것](#67-하지-못한-것--알려진-위험)에
따로 정리했다.

---

## 14. 프로젝트 종료 시 정리 체크리스트
...
- [ ] Bedrock Guardrail 삭제 — `671i24gxu4oo` (`maas-transit-guardrail-v3`)
